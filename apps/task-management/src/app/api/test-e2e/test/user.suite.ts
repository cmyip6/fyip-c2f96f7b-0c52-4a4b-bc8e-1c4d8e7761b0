...
export class UserE2eSpec extends NewBaseTest implements OnModuleInit {
  ...
  async userGetProfile(): Promise<void> {
    ...
    const { body: loginResponse } = await this.post(
      `/auth/login`,
      loginDto,
    ...
    const { body: profile } = await this.get(
      `profile`,
  ...
  async successLogin(): Promise<void> {
    // do login
    const { loginDto, user } =
      await this.factory.createUserWithNoPermissionsRole();
    const { body: loginResponse } = await this.post(
      `/auth/login`,
      loginDto,
    ).expect(HttpStatus.CREATED);
  ...
  async getUserPermissionProfiles(): Promise<void> {
    ...
    const { body: users } = await this.get(
      `permission-profiles`,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
  ...
  async getRoles(): Promise<void> {
    ...
    const {
      moduleUser: {
        id: moduleUserId,
        email: moduleUserEmail,
        roleId: moduleUserRoleId,
      },
    ...
    const { body: users } = await this.get(
      `roles`,
      jwtSuperAdminToken.accessToken,
    ).expect(HttpStatus.OK);
  ...
  async getUserSetting(): Promise<void> {
    ...
    const response = await this.post(`/auth/login`, loginDto).expect(
      HttpStatus.CREATED,
    );
    ...
    const { body: userSetting } = await this.get(
      `user-settings`,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
  ...
  async updateUserSetting(): Promise<void> {
    ...
    const { body: updatedSetting } = await this.get(
      `user-settings`,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
  ...
  async createSuperAdminUser(): Promise<{
    ...
    const { body: ret } = await this.post(
      `/auth/login`,
      superAdminUser.loginDto,
    ).expect(HttpStatus.CREATED);
    return {
      token: ret,
      superAdminUser: {
  ...
  async createGuestUser(): Promise<{
    token: TokenInterface;
    guestUser: GenericUserLoginDto & { id: string };
  }> {
  ...
  async createModuleUser(): Promise<{
    token: TokenInterface;
    moduleUser: GenericUserLoginDto & {
      id: string;
      roleId: number;
      firstName: string;
      lastName: string;
  ...
  async ggetFilteredUsers(): Promise<void> {
    ...
    const { body: allUsers } = await this.get(
      `filters`,
      superAdminUser.accessToken,
    ).expect(HttpStatus.OK);
  ...
  async getAllUserPermissions(): Promise<void> {
    ...
    const verifyPermissions = (
      ...
      const permissionConfig = {
        ...
        leader: {
          ...
          [EntityTypeOptions.Space]: () => true,
          [EntityTypeOptions.Folder]: (p: number) =>
            ![65536, 131072].includes(p),
          [EntityTypeOptions.Task]: () => true,
        },
        member: {
          [EntityTypeOptions.General]: (p: number) =>
            [8, 16, 32, 64].includes(p),
          [EntityTypeOptions.Space]: () => true,
          [EntityTypeOptions.Folder]: (p: number) =>
            ![65536, 131072].includes(p),
          [EntityTypeOptions.Task]: () => true,
    ...
    const { body: allPermissions } = await this.get(
      `/permission-manager/permission/all`,
      superUser.accessToken,
    ).expect(HttpStatus.OK);
    const superUserPermission = allPermissions.filter(
      (p) => p.userId === superUserId,
    );
    ...
    const superAdminUserPermission = allPermissions.filter(
      (p) => p.userId === superAdminUserId,
    );
    ...
    const ownerPermission = allPermissions.filter(
      (p) => p.userId === ownerUserId,
    );
    ...
    const modulePermission = allPermissions.filter(
      (p) => p.userId === moduleUserId,
    );
    ...
    const leaderPermission = allPermissions.filter(
      (p) => p.userId === leaderId,
    );
    ...
    const memberPermission = allPermissions.filter(
      (p) => p.userId === memberId,
    );
  ...
  async deactivateAndActivateUser(): Promise<void> {
    ...
    const {
      ...
      spaceId: spaceId1,
      workflowDB,
    } = await this.createFolder(null, jwtToken);
    const { folder: folder2, spaceId: spaceId2 } = await this.createFolder(
      null,
      jwtToken2,
    );
    const { folder: folder3, spaceId: spaceId3 } = await this.createFolder(
      null,
      jwtToken3,
    );

    this.logger.debug('update workflow to add approval constraint');
    const { body: workflow1 } = await this.get(
    ...
    const { body: wf3 } = await this.patch(
      `/workflow/module/${workflow1.id}`,
      updatedWorkflow,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
    expect(wf3).toBeDefined();
    const { body: updatedWorkflow1 } = await this.get(
      `/workflow/module/${workflow1.id}`,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
    ...
    await this.patch(
      `/task-action-assignee/${actionAssignee1.identifiers[0].id}`,
      { resolved: true },
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
    ...
    await this.post(
      `/task-action-assignee/task-action/${commentId2}`,
      addAssigneeDto,
      jwtToken.accessToken,
    ).expect(HttpStatus.CREATED);
    ...
    await this.patch(
      `/space/${spaceId3}/members`,
      updateMembersDto,
      jwtToken3.accessToken,
    ).expect(HttpStatus.OK);
    ...
    const completedSystemStage = systemStages.find(
      (el) => el.code === SystemStageCodeOptions.COMPLETED,
    );
    const completedState = updatedWorkflow1.states.find(
      (el) => el.systemStageId === completedSystemStage.id,
    );
    ...
    const { body: createdTag } = await this.post(
      `/tags/per-user`,
      fakeTag,
      jwtToken.accessToken,
    ).expect(HttpStatus.CREATED);
    this.logger.debug(`check tag exists`);
    const { body: tag } = await this.get(
      `/tags/per-user`,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
    ...
    const { body: automationLogs } = await this.post(
      `/automation/getmanylogs`,
      {},
      jwtToken.accessToken,
    ).expect(HttpStatus.CREATED);
    expect(automationLogs).toBeDefined();

    const customFieldDefinitionOptions =
      this.customFieldDefinitionFactory.fakeCreateCustomFieldDefinitionOptions();
    ...
    const fakeDashboard = this.dashboardFactory.fakeCreateDashboard(null, [
      folder1.id,
    ]);
    fakeDashboard.dashboardType = DashboardTypesOptions.My;

    const { body: dashboard } = await this.post(
      `/dashboard`,
      fakeDashboard,
      jwtToken.accessToken,
    ).expect(HttpStatus.CREATED);
    ...
    const { body: widgetCategory } = await this.post(
      `/widgets/categories`,
      fakeWidgetCategory,
      jwtToken.accessToken,
    ).expect(HttpStatus.CREATED);
    ...
    const fakeWidgetType = this.widgetFactory.fakeCreateWidgetType(
      widgetCategory.id,
      'tasksTable',
    );
    const { body: widgetType } = await this.post(
      `/widgets/types`,
      fakeWidgetType,
      jwtToken.accessToken,
    ).expect(HttpStatus.CREATED);
    ...
    const fakeWidget = this.widgetFactory.fakeCreateWidget(
      [folder1.id],
      widgetType.id,
      dashboard.id,
      null,
    ...
    const { body: widget } = await this.post(
      `/widgets`,
      fakeWidget,
      jwtToken.accessToken,
    ).expect(HttpStatus.CREATED);
    ...
    const { body: deactivated } = await this.post(
      `deactivate-user`,
      dto,
      jwtToken2.accessToken,
    ).expect(HttpStatus.CREATED);
    ...
    const { body: notFoundError } = await this.post(
      `deactivate-user`,
      dto,
      jwtToken2.accessToken,
    ).expect(HttpStatus.NOT_FOUND);
    expect(notFoundError.message).toBe(
      'User does not exist or is already deactivated',
    );
    this.logger.debug(
      `user should not be able to login or access any endpoint once deactivated`,
    );
    await this.get(`/tags/per-user`, jwtToken.accessToken).expect(
      HttpStatus.FORBIDDEN,
    );

    const { body: task1Db } = await this.get(
      `/task/${task.id}/folder/${folder1.id}`,
      jwtToken2.accessToken,
    ).expect(HttpStatus.OK);
    ...
    const { body: action1 } = await this.get(
      `/task-action-assignee/task-action/${commentId1}`,
      jwtToken2.accessToken,
    ).expect(HttpStatus.OK);
    ...
    const { body: action2 } = await this.get(
      `/task-action-assignee/task-action/${commentId2}`,
      jwtToken2.accessToken,
    ).expect(HttpStatus.OK);
    ...
    const { body: task2Db } = await this.get(
      `/task/${task2.id}/folder/${folder1.id}`,
      jwtToken2.accessToken,
    ).expect(HttpStatus.OK);
    ...
    const { body: task3Db } = await this.get(
      `/task/${task3.id}/folder/${folder1.id}`,
      jwtToken2.accessToken,
    ).expect(HttpStatus.OK);
    ...
    const { body: s1 } = await this.get(
      `/space/${spaceId1}`,
      jwtToken2.accessToken,
    ).expect(HttpStatus.OK);
    expect(s1.ownerId).toBe(userId2);

    const { body: f1 } = await this.get(
      `/folder/${folder1.id}`,
      jwtToken2.accessToken,
    ).expect(HttpStatus.OK);
    expect(f1.ownerId).toBe(userId2);

    const { body: s2 } = await this.get(
      `/space/${spaceId2}`,
      jwtToken2.accessToken,
    ).expect(HttpStatus.OK);
    ...
    const { body: activated } = await this.put(
      `activate-user/${userId1}`,
      dto,
      jwtToken2.accessToken,
    ).expect(HttpStatus.OK);
    ...
    expect(workflow.states[2].constraints.length).toBe(
      updatedWorkflow.states[2].constraints.length,
    );
    expect(workflow.states[2].constraints[0].swimlaneConstraint).toBeArrayEqual(
      swimlaneConstraint,
    );
    ...
    expect(workflow.states[1].approvalConstraint.dueIn).toBe(
      updatedWorkflow.states[1].approvalConstraint.dueIn,
    );
    ...
    expect(
      workflow.states[1].approvalConstraint.authorizedUserIds,
    ).toHaveLength(1);
  ...
  async createFolder(
    ...
    if (workflowId === null) {
      const workflow: WorkFlowEntity = await this.createWorkflowForFolder(
        jwtToken.accessToken,
      );
  ...
  async createAutomation(
    ...
    const { body: availableEvents } = await this.get(
      `/automation/getavailable`,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
    expect(availableEvents).toBeDefined();
    const available = availableEvents.availableActions.find(
      (a) => a.type === AutomationActionAddTaskCommentDto.name,
    );
    ...
    const { body: created } = await this.post(
      `/automation/createone`,
      automationBody,
      jwtToken.accessToken,
    ).expect(HttpStatus.CREATED);
    expect(created).toBeDefined();
    const { body: automationDB } = await this.get(
      `/automation/getone/automation/${created.id}`,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
  ...
  async sendUserInvitationWithSuperAdmin(): Promise<void> {
    ...
    const { body: user } = await this.get(
      invitedUser.id,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
