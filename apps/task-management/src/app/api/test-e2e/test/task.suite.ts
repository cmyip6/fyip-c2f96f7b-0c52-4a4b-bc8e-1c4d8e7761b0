...
import {
  ...
  WorkFlowResponseDto,
} from '../../src/dto/workflow';
import { RestoreTaskDto } from '../../src/dto/task/restore-task.dto';
import {
  ApprovalDueInTypeOptions,
  ApprovalEntityTypesOptions,
  ApprovalStatusOptions,
} from '../../src/module/approval-api-connector/enum';
...
import {
  AppConfigService,
  enqueueEventAndGetPromise,
  getQueueName,
  UserTypeOptions,
} from '@plexxis/plexxisjs-nest-libs';
import {
  APPROVAL_RETURN_CREATE_INSTANCE_JOB,
  APPROVAL_RETURN_QUEUE,
} from '../../src/module/approval-queue-handler/constants';
...
export class TaskE2eSpec extends NewBaseTest implements OnModuleInit {
  ...
  async checkInheritedPermissions(): Promise<void> {
    ...
    const { token: fakeJwtToken1 } =
      await this.userSuite.createSuperAdminUser();
    const fakeUser1Id = this.getUserIdFromAccessToken(
      fakeJwtToken1.accessToken,
    );

    const { token: fakeJwtToken2 } =
      await this.userSuite.createSuperAdminUser();
    const fakeUser2Id = this.getUserIdFromAccessToken(
      fakeJwtToken2.accessToken,
    );
    ...
    const { folder, spaceId } = await this.createFolder(
      null,
      jwtToken,
      workflow.id,
      spaceResponse.id,
    ...
    const { folder: subFolder } = await this.createFolder(
      folder.id,
      jwtToken,
      workflow.id,
      spaceResponse.id,
    ...
    const { folder: subSubFolder } = await this.createFolder(
      subFolder.id,
      jwtToken,
      workflow.id,
      spaceResponse.id,
    ...
    let folderTree = (
      await this.get(`/folder/folder-tree`, jwtToken.accessToken)
    ).body as FolderTreeDto[];

    // get inherited permissions of folder (fakeUserId should be there)
    //folder
    let hasInheritedPermission: boolean =
      folderTree[0].children[0].members?.some(
    ...
    hasInheritedPermission =
      folderTree[0].children[0].children[0].members?.some(
        (member) => member.userId === fakeUser1Id && member.inherit,
      );
    expect(hasInheritedPermission).toBe(true);
    ...
    //last child
    hasInheritedPermission =
      folderTree[0].children[0].children[0].children[0].members?.some(
        (member) => member.userId === fakeUser1Id && member.inherit,
      );
    expect(hasInheritedPermission).toBe(true);
    ...
    await this.patch(
      ...
      { viewType: FolderViewTypeOptions.PRIVATE },
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
    ...
    folderTree = (
      await this.get(
        `/folder/folder-tree?space-ids=${spaceResponse.id}`,
        jwtToken.accessToken,
      )
    ).body as FolderTreeDto[];

    // get inherited permissions of subfolder1 (fakeUserId shouldn't be there)
    hasInheritedPermission = folderTree[0].children[0].members?.some(
      (member) => member.userId === fakeUser1Id && member.inherit,
    );
  ...
  async multipleTasksFollowers(): Promise<void> {
    ...
    const { body: f1DB } = await this.get(
      `/folder/${f1.id}`,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
    ...
    const { body: response } = await this.post(
      `/folder/follow/${f1.id}/${fakeUser1Id}`,
      {},
      jwtToken.accessToken,
    ).expect(HttpStatus.CREATED);
    ...
    const { body: response1 } = await this.post(
      `/folder/follow/${f1.id}/${fakeUser2Id}`,
      {},
      jwtToken.accessToken,
    ).expect(HttpStatus.CREATED);
    ...
    this.logger.debug(
      'Followers of folders should be added to task followers list',
    );
    const { body: taskFollowers } = await this.get(
      `follow/${task.id}/folder/${f1.id}`,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
  ...
  async createFOSourceTask(): Promise<void> {
    ...
    const { body: task } = await this.post(
      ...
      {
        ...fakeTask,
        source: FINANCIAL_OUTLOOK,
        assignees: [userId],
        taskType: TaskTypeOptions.FINANCIAL,
      },
      jwtToken.accessToken,
    ).expect(HttpStatus.CREATED);
    ...
    expect(task.startDate).toBeDateEqualWithoutMilliseconds(
      fakeTask.startDate.toISOString(),
    );
  }
  ...
  async updateTaskAssignee(): Promise<void> {
    ...
    await this.patch(`assignees/${task.id}`, data, jwtToken.accessToken).expect(
      HttpStatus.OK,
    );

    this.logger.debug('get task with new assignees');
    const { body: taskWithAssignee } = await this.get(
      `${task.id}/folder/${folder.id}`,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
    ...
    const assigneeAdded = taskActionAssigneeAdded.data.find(
      (a) =>
        a.action === 'assign' &&
        a.parameters.assignees.added.length === 1 &&
        a.parameters.assignees.added.includes(userId),
    );
    ...
    const { body: taskWithoutAssignee } = await this.get(
      `${task.id}/folder/${folder.id}`,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
    ...
    const assigneeRemoved = taskActionAssigneeRemoved.data.find(
      (a) =>
        a.action === 'assign' &&
        a.parameters.assignees.removed.length === 1 &&
        a.parameters.assignees.removed.includes(userId),
    );
    ...
    const randomUUIDsArray = Array.from({ length: 1001 }, () =>
      faker.string.uuid(),
    );
    data = {
      folderId: folder.id,
  ...
  async CreateTaskAndCheckAction(): Promise<void> {
    ...
    this.logger.debug('validate we have task action with the added assignee');
    const { body: taskAction } = await this.post(
      `/task-action/folder/${folder.id}/task/${task.id}`,
      {},
      jwtToken.accessToken,
    ).expect(HttpStatus.CREATED);
    ...
    const exists = taskAction.data.find(
      (x: { action: string; taskId: number }) =>
        x.action === TaskActionOptions.CREATE && x.taskId === task.id,
    );
  ...
  async CreateTaskAndCheckSpaceId(): Promise<void> {
    ...
    this.logger.debug('validate we have task action with the added assignee');
    const { body: getOneTask } = await this.get(
      `${task.id}/folder/${folder.id}`,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
  ...
  public async getSingleTask(
    ...
    const { body: task } = await this.get(
      `${taskId}/folder/${folderId}`,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
  ...
  async CheckAutoChildTaskAssignee(): Promise<void> {
    ...
    const fakeChildTask = this.factory.fakeCreateTask(
      userId,
      folder.id,
      task.id,
    );
    const { body: childTask } = await this.post(
      ``,
      fakeChildTask,
      jwtToken.accessToken,
    ).expect(HttpStatus.CREATED);
    ...
    const taskWithAssignee = await this.getSingleTask(
      task.id,
      folder.id,
      jwtToken,
    );
    expect(taskWithAssignee.assignees.length).toBe(1);
    expect(taskWithAssignee.assignees[0]).toBe(userId);
    const childTaskResponse = await this.getSingleTask(
      childTask.id,
      folder.id,
      jwtToken,
    );
  ...
  async fetchTaskWithAllChildren(): Promise<void> {
    ...
    const { body: parentTask } = await this.post(
      '',
      parentTaskPayload,
      jwtToken.accessToken,
    ).expect(HttpStatus.CREATED);
    ...
    const childTaskPayload = this.factory.fakeCreateTask(
      userId,
      folder.id,
      parentTask.id,
    );
    ...
    this.logger.debug('Creating child task...');
    const childTaskPayload2 = this.factory.fakeCreateTask(
      userId,
      folder.id,
      parentTask.id,
    );
    const { body: childTask2 } = await this.post(
      '',
      childTaskPayload2,
      jwtToken.accessToken,
    ).expect(HttpStatus.CREATED);
    ...
    const grandChildTaskPayload = this.factory.fakeCreateTask(
      userId,
      folder.id,
      childTask.id,
    );
    const { body: grandChildTask } = await this.post(
      '',
      grandChildTaskPayload,
      jwtToken.accessToken,
    ).expect(HttpStatus.CREATED);

    this.logger.debug(
      'Fetching task with all associated children from database...',
    );
    ...
    fetchedTask.children.map((child) =>
      expect(child.importanceId).toBeDefined(),
    );
    ...
    this.logger.debug(
      `Asserting grandchild task ID: Expected ${grandChildTask.id}`,
    );
   ...
   */
  @Test('Get Assigned to me TaskEntity')
  async getAssignedTask(): Promise<void> {
    ...
    const { folder: folder2, spaceId: spaceId2 } = await this.createFolder(
      null,
      myJwtToken,
    );
    this.logger.debug('get user id');
    ...
    const workflowStates = await this.workflowStateFactory.repository.find({
      where: { workflowId: workflowDB.id },
    });

    this.logger.debug('validate my task 2');
    const { body: taskWithAssigneeF2 } = await this.get(
      `${task2.id}/folder/${folder2.id}`,
      myJwtToken.accessToken,
    ).expect(HttpStatus.OK);
    ...
    this.logger.debug(
      'add another task to folder with different assignee to folder ',
    );
    const fakeChildTaskData = this.factory.fakeCreateTask(
      fakeUserId,
      folder.id,
    );
    const { body: fakeChildTask } = await this.post(
      ``,
      fakeChildTaskData,
      fakeJwtToken.accessToken,
    ).expect(HttpStatus.CREATED);
    this.logger.debug('add user as assignee to task');
    ...
    await this.patch(
      `assignees/${fakeChildTask.id}`,
      data,
      fakeJwtToken.accessToken,
    ).expect(HttpStatus.OK);
    const { body: taskWithAssignee2 } = await this.get(
      `${fakeChildTask.id}/folder/${folder.id}`,
      fakeJwtToken.accessToken,
    ).expect(HttpStatus.OK);
    ...
    expect(taskAssignedToMe.tasks[0].assignees[0]).toBe(myUserId);
    expect(
      taskAssignedToMeSpace.tasks.every((el) => el.spaceId === spaceId2),
    ).toBeTruthy();
    ...
    this.logger.debug(
      'send 1000 stateIds to get tasks, should fail with Bad Request',
    );
    await this.post(
      `assigned`,
      {
  ...
  async automationUserAddComment(): Promise<void> {
    ...
    expect(folderTree[0].children[0]).toMatchObject({
      id: folder.id,
      title: folder.title,
    });
    ...
    const automationUser = await this.logUser(
      this.userFactory.getPasServiceUserLoginDto(),
      UserTypeOptions.MODULE_USER,
    );
    const commentDto: CreateTaskActionDto = {
      comment: 'this is a test comment',
      mentionMembers: [userId],
    };
    ...
    const { body: commentGet } = await this.post(
      `/task-action/folder/${folder.id}/task/${task.id}`,
      {},
      jwtToken.accessToken,
    ).expect(HttpStatus.CREATED);
    ...
    const commentAction = commentGet.data.find(
      (a) => a.action === 'comment' && a.id === respComment.identifiers[0].id,
    );
   ...
   * @returns {Promise<void>} - A Promise that resolves when the automation is completed successfully.
   */
  @Test('Get Automations TaskEntity By Folder')
  ...
  async createCustomFieldOnTask(): Promise<void> {
    ...
    await this.post(``, fakeTask2, jwtToken.accessToken).expect(
      HttpStatus.CREATED,
    );
    const customFieldDefinitionDto =
      this.customFieldDefinitionFactory.fakeCreateCustomFieldDefinition(
        [],
        CustomFieldDefinitionTypeOptions.WEBSITE,
      );
    const customFieldDefinitionDto2 =
      this.customFieldDefinitionFactory.fakeCreateCustomFieldDefinition(
        [],
    ...
    const { body: customFields } = await this.get(
      `/custom-field-definition/${spaceId}?show-inactive=false`,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
    expect(customFields.length).toBe(1);
    const createdCF = customFields.find(
      (cf: { id: number }) => cf.id === customField.identifiers[0].id,
    );
    ...
    const createdCF2 = customFields2DB.find(
      (cf: { id: number }) => cf.id === customField2.identifiers[0].id,
    );
    ...
    const { body: taskCustomFields } = await this.get(
      `custom-field/folder/${folder.id}/task/${task.id}`,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
    const customFieldWithValue = taskCustomFields.customFields.find(
      (cf: { value: string }) => cf.value === customFieldValue,
    );
    expect(customFieldWithValue).toBeDefined();
    expect(
      taskCustomFields.customFields.some(
        (cf) => cf.customFieldDefinitionId === customField2.identifiers[0].id,
      ),
    ).toBe(true);

    await this.post(
      `custom-field/${customFieldWithValue.id}/folder/${folder.id}/task/${task.id}/pin`,
      {},
      jwtToken.accessToken,
    ).expect(HttpStatus.CREATED);

    const { body: taskCustomFields2 } = await this.get(
      `custom-field/folder/${folder.id}/task/${task.id}`,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
    const { isPinned } =
      taskCustomFields2.customFields.find(
        (cf: { value: string }) => cf.value === customFieldValue,
      ) ?? {};
    ...
    const { body: taskCustomFields3 } = await this.get(
      `custom-field/folder/${folder.id}/task/${task.id}`,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
    const { isPinned: isPinned2 } =
      taskCustomFields3.customFields.find(
        (cf: { value: string }) => cf.value === customFieldValue,
      ) ?? {};
  ...
  async addMultiUserCustomFieldOnTask(): Promise<void> {
    ...
    const { body: taskCustomFields1 } = await this.get(
      `custom-field/folder/${folder.id}/task/${task.id}`,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
    const customFieldWithValue1 = taskCustomFields1.customFields.find(
      (cf: { value: string }) => cf.value === customFieldValue1,
    );
    ...
    const customFieldWithValue2 = taskCustomFields2.customFields.find(
      (cf: { value: string }) => cf.value === customFieldValue2,
    );
    ...
    expect(responseFail.message).toBe(
      `Custom field value for ${CustomFieldDefinitionTypeOptions.USER} is invalid`,
    );
  }
  ...
  async createTaskCustomFieldAssigneeOnTask(): Promise<void> {
    ...
    this.logger.debug(
      'create task should fail if properties in assignees are not UUIDs',
    );
    fakeTask['assignees'] = ['1234'];
    await this.post(``, fakeTask, jwtToken.accessToken).expect(
      HttpStatus.BAD_REQUEST,
    );
    ...
    expect(taskCustomFields.customFields[0].customFieldDefinitionId).toBe(
      customField.identifiers[0].id,
    );
  ...
  async updateTaskAssigneesPosition(): Promise<void> {
    ...
    const { body: taskDetailBefore } = await this.get(
      `${task.id}/folder/${folder.id}`,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
    ...
    const { body: response1 } = await this.patch(
      `assignee/position`,
      taskPosition,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
    ...
    const { body: taskDetailAfter1 } = await this.get(
      `${task.id}/folder/${folder.id}`,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
    expect(taskDetailAfter1.id).toBe(task.id);
    expect(taskDetailAfter1.assignees).toStrictEqual([
      userId1,
      userId2,
      userId3,
      userId5,
    ...
    this.logger.debug('update task assignee position and verify');
    const { body: response2 } = await this.patch(
      `assignee/position`,
      taskPosition,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
    ...
    const { body: taskDetailAfter2 } = await this.get(
      `${task.id}/folder/${folder.id}`,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
    expect(taskDetailAfter2.id).toBe(task.id);
    expect(taskDetailAfter2.assignees).toStrictEqual([
      userId1,
      userId2,
      userId3,
      userId4,
    ...
    this.logger.debug('update task assignee position and verify');
    const { body: response3 } = await this.patch(
      `assignee/position`,
      taskPosition,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
    ...
    const { body: taskDetailAfter3 } = await this.get(
      `${task.id}/folder/${folder.id}`,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
    expect(taskDetailAfter3.id).toBe(task.id);
    expect(taskDetailAfter3.assignees).toStrictEqual([
      userId4,
      userId1,
      userId2,
      userId3,
    ...
    this.logger.debug('update task assignee position and verify');
    const { body: response4 } = await this.patch(
      `assignee/position`,
      taskPosition,
      jwtToken.accessToken,
    ).expect(HttpStatus.BAD_REQUEST);
    expect(response4.message).toBe(
      `Assignee ${taskPosition.userId} does not exist on task ${task.id}`,
    );
  }
  ...
  async getTaskFollowers(): Promise<void> {
    ...
    const { body: followers } = await this.get(
      `follow/${task.id}/folder/${folder.id}`,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
  ...
  async unFollowTask(): Promise<void> {
    ...
    await this.delete(
      `unfollow/${folder.id}/${task.id}/${userId}`,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
    const { body: followers } = await this.get(
      `follow/${task.id}/folder/${folder.id}`,
  ...
  async deleteTaskById(): Promise<void> {
    ...
    const fakeTask = this.factory.fakeCreateTask(
      userId,
      folder.id,
      parentTask.id,
    );
    ...
    const taskAssigneeDto: TaskAssigneesDto = {
      assignees: [userId],
      folderId: folder.id,
    };
    await this.patch(
    ...
    const customFieldIds = await this.createManyCustomFields(
      2,
      CustomFieldDefinitionTypeOptions.TEXT,
      jwtToken.accessToken,
      spaceId,
    ...
    const addApprovalToTaskDto =
      this.approvalFactory.fakeCreateAddApprovalToTaskDto([userId]);
    ...
    const { body: wbDB } = await this.get(
      `/workflow/module/${workflowDB.id}`,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
    ...
    const { body: wbDbUpdated } = await this.get(
      `/workflow/module/${workflowDB.id}`,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);

    const approvalConstraintInstanceRepo = this.dataSource.getRepository(
      ApprovalConstraintInstanceEntity,
    );
    ...
    await this.get(
      `${childTask.id}/folder/${folder.id}`,
      jwtToken.accessToken,
    ).expect(HttpStatus.NOT_FOUND);
    await this.get(
      `${parentTask.id}/folder/${folder.id}`,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
  }
  ...
  async moveApprovalContraintTaskAfterConstraitRemoved(): Promise<void> {
    ...
    const { body: updatedWorkflow } = await this.get(
      `/workflow/module/${workflowDB.id}`,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
    ...
    const fakeUpdatedWorkflow2 =
      this.workflowFactory.fakeUpdateWorkflow(updatedWorkflow);
    ...
    await this.patch(
      `/workflow/module/${workflowDB.id}`,
      fakeUpdatedWorkflow2,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
    const { body: updatedWorkflow2 } = await this.get(
      `/workflow/module/${workflowDB.id}`,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
  ...
  async createTaskInConstraintWorkflow(): Promise<void> {
    ...
    this.logger.debug('create task');
    const { body: task1 } = await this.post(
      ``,
      fakeTask1,
      jwtToken.accessToken,
    ).expect(HttpStatus.CREATED);
    ...
    this.logger.debug(`Update workflow`);
    const fakeUpdatedWorkflow = this.workflowFactory.fakeUpdateWorkflow(wbDB);
    fakeUpdatedWorkflow.states[1].constraints = [
      {
        swimlaneConstraint: [fakeUpdatedWorkflow.states[0].code],
    ...
    const { body: wbDB2 } = await this.get(
      `/workflow/module/${workflowDB.id}`,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
    ...
    const { body: task3 } = await this.post(
      ``,
      fakeTask3,
      jwtToken.accessToken,
    ).expect(HttpStatus.BAD_REQUEST);
    ...
    const { body: wbDB3 } = await this.get(
      `/workflow/module/${workflowDB.id}`,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
    ...
    const { body: task5 } = await this.post(
      ``,
      fakeTask5,
      jwtToken.accessToken,
    ).expect(HttpStatus.BAD_REQUEST);
  ...
  async deleteManyTasks(): Promise<void> {
    ...
    await this.get(
      `${task1.id}/folder/${folder.id}`,
      jwtToken.accessToken,
    ).expect(HttpStatus.NOT_FOUND);
    await this.get(
      `${task2.id}/folder/${folder.id}`,
      jwtToken.accessToken,
    ).expect(HttpStatus.NOT_FOUND);
    ...
    const task1FromList = body.data.find(
      (task: { id: string }) => task.id === task1.id,
    );
    expect(task1FromList).toBeDefined();
    expect(task1FromList.deleteReason).toBe(expectedReason1);
    const task2FromList = body.data.find(
      (task: { id: string }) => task.id === task2.id,
    );
  ...
  async softDeleteTask(): Promise<void> {
    ...
    await this.delete(
      `delete/${task.id}/folder/${folder.id}?reason=${badRequetReason}`,
      jwtToken.accessToken,
    ).expect(HttpStatus.BAD_REQUEST);
    const reason = faker.string.alphanumeric(200);
    await this.delete(
      `delete/${task.id}/folder/${folder.id}?reason=${reason}`,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
    await this.get(
      `${task.id}/folder/${folder.id}`,
    ...
    const taskFromList = body.data.find(
      (task: { id: string }) => task.id === task.id,
    );
    ...
    await this.delete(
      `delete/${task2.id}/folder/${folder.id}`,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
  ...
  async archiveManyTasks(): Promise<void> {
    ...
    const fakeSubTask3 = this.factory.fakeCreateTask(
      userId,
      folder.id,
      task1.id,
    );
    const { body: subtask3 } = await this.post(
      ``,
      fakeSubTask3,
      jwtToken.accessToken,
    ).expect(HttpStatus.CREATED);
    ...
    const task1FromList = response.data.find(
      (task: { id: string }) => task.id === task1.id,
    );
    expect(task1FromList).toBeDefined();
    expect(task1FromList.pathStr).toHaveLength(3);
    expect(task1FromList.pathStr[0].type).toBe(FolderTypeOptions.SPACE);
    expect([FolderTypeOptions.FOLDER, FolderTypeOptions.PROJECT]).toContain(
      task1FromList.pathStr[1].type,
    );
    ...
    const task2FromList = response.data.find(
      (task: { id: string }) => task.id === task2.id,
    );
    expect(task2FromList).toBeDefined();
    expect(task2FromList.archiveReason).toBe(expectedReason2);
    this.logger.debug(
      'Archived task should throw bad request exception if the task had already been archvied',
    );
    ...
    expect(BadRequestResponse.message).toBe(
      `Task with id:${task1.id} had already been archived.`,
    );
    ...
    expect(BadRequestResponse2.message).toBe(
      `Task with id:${task2.id} had already been archived.`,
    );
    ...
    expect(BadRequestResponse3.message).toBe(
      `Task with id:${subtask3.id} had already been archived.`,
    );
  }
  ...
  async addTagToTask(): Promise<void> {
    ...
    await this.post(
      `tag/${task.id}/${tag.id}/${folder.id}`,
      {},
      jwtToken.accessToken,
    ).expect(HttpStatus.CREATED);
    const { body: taskResponse } = await this.get(
      `${task.id}/folder/${folder.id}`,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
  ...
  async uploadTaskDescriptionImage(): Promise<void> {
    ...
    const { body: updatedTask } = await this.patch(
      `${task.id}`,
      fakeUpdateTask,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
    ...
    const responsePatch = await this.get(
      `/task-attachment/${matchPatch[2]}`,
      null,
      { [`${COOKIE_KEY}`]: jwtToken.accessToken },
    )
      .responseType('blob')
      .expect(HttpStatus.OK);
    ...
    const { body: updatedTaskDB } = await this.get(
      `${task.id}/folder/${folder.id}`,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
    ...
    const responseGet = await this.get(
      `/task-attachment/${matchGet[2]}`,
      null,
      { [`${COOKIE_KEY}`]: jwtToken.accessToken },
    )
      .responseType('blob')
    ...
    const { body: attachments } = await this.get(
      `/task-attachment/folder/${folder.id}/task/${task.id}`,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
  ...
  async uploadAndRemoveTaskDescriptionImage(): Promise<void> {
    ...
    const { body: attachments2 } = await this.get(
      `/task-attachment/folder/${folder.id}/task/${task.id}`,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
  ...
  async removeTagFromTask(): Promise<void> {
    ...
    const { body: response } = await this.get(
      `${task.id}/folder/${folder.id}`,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
    ...
    expect(notFoundResponse.message).toBe(
      `Label '${tag.title}' is not found in the task.`,
    );
  }

  @Test('Add tag to task should fail at constraint')
  async tagTaskFolderConstraint(): Promise<void> {
    const repoTagTaskFolder =
      this.dataSource.getRepository(TagTaskFolderEntity);
    this.logger.debug('create user and folder');
    const { folder, jwtToken, spaceId } = await this.createFolder();
    const userId = this.getUserIdFromAccessToken(jwtToken.accessToken);
    const fakeTask = this.factory.fakeCreateTask(userId, folder.id);
  ...
  @Test('Get Multiple TaskEntity By Ids')
  async getMultipleTasksByIds(): Promise<void> {
    this.logger.debug('create user and folder');
    const { folder, jwtToken, spaceId } = await this.createFolder();
    const { folder: folder2, spaceId: space2Id } = await this.createFolder(
      null,
      jwtToken,
    );
    const userId = this.getUserIdFromAccessToken(jwtToken.accessToken);
    ...
    await this.post(``, fakeTask5, jwtToken.accessToken).expect(
      HttpStatus.CREATED,
    );
  ...
  async getPrevAndNextTaskId(): Promise<void> {
    ...
    this.logger.debug(
      `First task details: prevTask ID = ${task1Response.prevTaskId}`,
    );
    ...
    this.logger.debug(
      `Second task details: nextTask ID = ${task2Response.nextTaskId}`,
    );
    ...
    this.logger.debug(
      `Third task details: nextTask ID = ${task3Response.nextTaskId}`,
    );
    ...
    this.logger.debug(
      `Second task details (revisited): prevTask ID = ${task2Resp.prevTaskId}, nextTask ID = ${task2Resp.nextTaskId}`,
    );

    this.logger.debug('no view passed no return of prev and next task');
    const { body: task2RespWithOutView } = await this.get(
      `${task2.id}/folder/${folder.id}`,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
    ...
    const { body: task6 } = await this.post(
      ``,
      fakeTask6,
      jwtToken.accessToken,
    ).expect(HttpStatus.CREATED);
    ...
    this.logger.debug(
      'patch request should fail with a not found error instead of type error',
    );
    await this.patch(
      `position/${task6.id}`,
      moveTaskDto,
    ...
    const { body: task5DB } = await this.get(
      `${task5.id}/folder/${folder.id}`,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
    ...
    const { body: task7 } = await this.post(
      ``,
      { ...fakeTask7, workflowStateId: workflowStates[2].id },
      jwtToken.accessToken,
    ).expect(HttpStatus.CREATED);
    const fakeTask8 = this.factory.fakeCreateTask(userId, folder.id);
    const { body: task8 } = await this.post(
      ``,
      { ...fakeTask8, workflowStateId: workflowStates[2].id },
      jwtToken.accessToken,
    ).expect(HttpStatus.CREATED);
    const fakeTask9 = this.factory.fakeCreateTask(userId, folder.id);
    const { body: task9 } = await this.post(
      ``,
      { ...fakeTask9, workflowStateId: workflowStates[2].id },
      jwtToken.accessToken,
    ).expect(HttpStatus.CREATED);
  ...
  async archiveTask(): Promise<void> {
    ...
    await this.post(
      `archive/${task.id}/folder/${folder.id}`,
  ...
  async archiveUnarchiveTaskModifyFolderDates(): Promise<void> {
    ...
    fakeTask.duration = moment(fakeTask.endDate).diff(
      moment(fakeTask.startDate),
      'days',
    );
    ...
    fakeLaterTask.endDate = moment(fakeLaterTask.startDate)
      .add(10, 'days')
      .toDate();
    fakeLaterTask.duration = moment(fakeLaterTask.endDate).diff(
      moment(fakeLaterTask.startDate),
      'days',
    );
    ...
    await this.post(``, fakeLaterTask, jwtToken.accessToken).expect(
      HttpStatus.CREATED,
    );

    this.logger.debug('check folder start and endDate');
    const { body: folderDB } = await this.get(
      `/folder/${folder.id}`,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
    expect(
      moment(folderDB.startDate).toISOString(),
    ).toBeDateEqualWithoutMilliseconds(
      moment(fakeTask.startDate).toISOString(),
    );
    expect(
      moment(folderDB.endDate).toISOString(),
    ).toBeDateEqualWithoutMilliseconds(
      moment(fakeLaterTask.endDate).toISOString(),
    );
    this.logger.debug('archive task by id');
    await this.post(
      `archive/${task.id}/folder/${folder.id}`,
    ...
    const { body: folderDB2 } = await this.get(
      `/folder/${folder.id}`,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
    expect(folderDB2.startDate).toBeDateEqualWithoutMilliseconds(
      moment(fakeLaterTask.startDate).toISOString(),
    );
    expect(
      moment(folderDB2.endDate).toISOString(),
    ).toBeDateEqualWithoutMilliseconds(
      moment(fakeLaterTask.endDate).toISOString(),
    );
    this.logger.debug('restore earlier task');
    await this.post(
      `archive/restore/${task.id}/folder/${folder.id}`,
      { childrenTaskIds: [] },
      jwtToken.accessToken,
    ).expect(HttpStatus.CREATED);
    this.logger.debug('check folder start and endDate after unarchive');
    const { body: folderDB3 } = await this.get(
      `/folder/${folder.id}`,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
    expect(folderDB3.startDate).toBeDateEqualWithoutMilliseconds(
      moment(fakeTask.startDate).toISOString(),
    );
    expect(
      moment(folderDB3.endDate).toISOString(),
    ).toBeDateEqualWithoutMilliseconds(
      moment(fakeLaterTask.endDate).toISOString(),
    );
  }
   ...
   * @returns {Promise<void>} A Promise that resolves to undefined.
   */
  @Test('Archive and UnArchive TaskEntity')
  async archiveAndUnArchiveTask(): Promise<void> {
    ...
    const fakeSubTask1 = this.factory.fakeCreateTask(
      userId,
      folder.id,
      task.id,
    );
    const fakeSubTask2 = this.factory.fakeCreateTask(
      userId,
      folder.id,
      task.id,
    );
    const { body: subTask1 } = await this.post(
      ``,
      fakeSubTask1,
      jwtToken.accessToken,
    ).expect(HttpStatus.CREATED);
    const { body: subTask2 } = await this.post(
      ``,
      fakeSubTask2,
      jwtToken.accessToken,
    ).expect(HttpStatus.CREATED);
    ...
    await this.post(
      `archive/${subTask2.id}/folder/${folder.id}`,
      { archiveReason: 'Archive this Task' },
      jwtToken.accessToken,
    ).expect(HttpStatus.CREATED);
    ...
    const taskDb = await repo.findOneBy({
      id: task.id,
      archivedWhy: 'Archive this Task',
    });
    const subtask1Db = await repo.findOneBy({
      id: subTask1.id,
      archivedWhy: 'Archive this Task',
    });
    const subtask2Db = await repo.findOneBy({
      id: subTask2.id,
      archivedWhy: 'Archive this Task',
    });
    ...
    const archived = taskAction.data.find(
      (x: { action: string; taskId: number }) =>
        x.action === TaskActionOptions.ARCHIVE && x.taskId === task.id,
    );
    ...
    expect(subtask1DbBeforeRestore.archivedGroupId).not.toBe(
      subtask2DbBeforeRestore.archivedGroupId,
    );
    ...
    const { body: taskAction2 } = await this.post(
      `/task-action/folder/${folder.id}/task/${task.id}`,
      {},
      jwtToken.accessToken,
    ).expect(HttpStatus.CREATED);
    ...
    const unarchived = taskAction2.data.find(
      (x: { action: string; taskId: number }) =>
        x.action === TaskActionOptions.UNARCHIVE && x.taskId === task.id,
    );
   ...
   * @returns {Promise<void>} A Promise that resolves to undefined.
   */
  @Test('Get Archived Deleted TaskEntity Subtasks')
  async getArchivedDeletedTasksSubtasks(): Promise<void> {
    ...
    await this.post(
      `archive/${task1.id}/folder/${folder.id}`,
      { archiveReason: 'Archive this Task' },
      jwtToken.accessToken,
    ).expect(HttpStatus.CREATED);
    ...
    const archivedSubtask1 = archivedTask.children.find(
      (s) => s.id === subTask1.id,
    );
    expect(archivedSubtask1).toBeDefined();
    expect(archivedSubtask1.title).toBe(subTask1.title);
    const archivedSubtask2 = archivedTask.children.find(
      (s) => s.id === subTask2.id,
    );
    ...
    const fakeSubTask4 = this.factory.fakeCreateTask(
      userId,
      folder.id,
      task2.id,
    );
    ...
    const { body: subTask4 } = await this.post(
      ``,
      fakeSubTask4,
      jwtToken.accessToken,
    ).expect(HttpStatus.CREATED);
    ...
    const deleteSubtask1 = deletedTask.children.find(
      (s) => s.id === subTask3.id,
    );
    expect(deleteSubtask1).toBeDefined();
    expect(deleteSubtask1.title).toBe(subTask3.title);
    const deleteSubtask2 = deletedTask.children.find(
      (s) => s.id === subTask4.id,
    );
  ...
  async restoreDeletedTask(): Promise<void> {
    ...
    const { body: subTask } = await this.post(
      ``,
      fakeSubTask,
      jwtToken.accessToken,
    ).expect(HttpStatus.CREATED);

    const fakeSubSubTask = this.factory.fakeCreateTask(
      userId,
      folder.id,
      subTask.id,
    );
    const { body: subSubTask } = await this.post(
      ``,
      fakeSubSubTask,
      jwtToken.accessToken,
    ).expect(HttpStatus.CREATED);
    ...
    this.logger.debug('delete task by id');
    await this.delete(
      `delete/${task.id}/folder/${folder.id}`,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
    await this.get(
      `${task.id}/folder/${folder.id}`,
    ...
    await this.get(
      `${subTask.id}/folder/${folder.id}`,
      jwtToken.accessToken,
    ).expect(HttpStatus.NOT_FOUND);
    await this.get(
      `${subSubTask.id}/folder/${folder.id}`,
      jwtToken.accessToken,
    ).expect(HttpStatus.NOT_FOUND);
    ...
    const { body: response2 } = await this.get(
      `${subSubTask.id}/folder/${folder.id}`,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
   ...
   * @returns {Promise<void>} A promise that resolves when the operation is complete.
   */
  @Test('Get Many Archived TaskEntity')
  ...
  async updateTask(): Promise<void> {
    ...
    await this.patch(`${task.id}`, fakeUpdateTask, jwtToken.accessToken).expect(
      HttpStatus.OK,
    );
    ...
    expect(response.startDate).toBeDateEqualWithoutMilliseconds(
      fakeUpdateTask.startDate,
    );
    expect(response.endDate).toBeDateEqualWithoutMilliseconds(
      fakeUpdateTask.endDate,
    );
    ...
    const newStartDate = moment(fakeUpdateTask.endDate)
      .subtract(1, 'days')
      .toDate();
    await this.patch(
      `${task.id}`,
    ...
    expect(response2.endDate).toBeDateEqualWithoutMilliseconds(
      fakeUpdateTask.endDate,
    );
    this.logger.debug(
      'update with start date > end date should throw 400 exception',
    );
    await this.patch(
      `${task.id}`,
      {
        startDate: moment(fakeUpdateTask.endDate).add(1, 'days').toDate(),
        folderId: folder.id,
      },
      jwtToken.accessToken,
    ).expect(HttpStatus.BAD_REQUEST);
    ...
    const updateAction = taskAction.data.find(
      (x: { action: string; taskId: number }) =>
        x.action === TaskActionOptions.UPDATE && x.taskId === task.id,
    );
    expect(updateAction).toBeDefined();
    const descriptionProp = updateAction.parameters.updates.find(
      (update) => update.property === 'description',
    );
  ...
  async updateTaskEndDAte(): Promise<void> {
    ...
    const fakeTask1 = this.factory.fakeCreateTask(
      userId,
      folder.id,
      undefined,
      undefined,
    ...
    fakeUpdateTask1['endDate'] = faker.date.between({
    ...
    const fakeTask2 = this.factory.fakeCreateTask(
      userId,
      folder.id,
      task1.id,
      undefined,
    ...
    const fakeUpdateTask2 = this.factory.fakeUpdateTask(folder.id);
    fakeUpdateTask2['endDate'] = faker.date.between({
      from: fakeUpdateTask1.endDate,
      to: folder.endDate,
    });
    ...
    await this.patch(
      ...
      {
        folderId: folder.id,
        endDate: faker.date.future({ years: 1, refDate: task2.endDate }),
      },
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
    this.logger.debug(
      'Task update in task action should not contain null value in the JSON array',
    );
    ...
    expect(
      allTaskAction.every((actions) =>
        actions.parameters.updates.every(
  ...
  async updateTaskImportance(): Promise<void> {
    ...
    expect(task).toBeDefined();
    const allImportance = await this.dataSource
      .getRepository(ImportanceEntity)
      .find();
    expect(allImportance.length).toBeGreaterThan(1);
    const updateImportanceId = allImportance.find(
      (el) => el.id !== task.importanceId,
    )?.id;
    ...
    const { body: updatedTask } = await this.get(
      `${task.id}/folder/${folder.id}`,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
  ...
  async updateTaskStartDate(): Promise<void> {
    ...
    const earlierStartDate = moment
      .utc(folder.startDate)
      .subtract(1, 'days')
      .toDate();
    await this.patch(
      `${task.id}`,
      { startDate: earlierStartDate, folderId: folder.id },
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
    const { body: response2 } = await this.get(
      `${task.id}/folder/${folder.id}`,
    ...
    expect(response2.startDate).toBeDateEqualWithoutMilliseconds(
      earlierStartDate,
    );
  }
  ...
  async updateTaskImportanceWithNotification(): Promise<void> {
    ...
    const notifcationPromise = enqueueEventAndGetPromise(
      ...
      (job, data) =>
        job?.data?.data?.taskId === data?.taskId &&
        job?.data?.eventName === data?.event,
    );

    this.logger.debug('Update importance');
    await this.patch(
      `${task.id}`,
      { folderId: folder.id, importanceId: updateImportanceId },
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
    const { body: updatedTask } = await this.get(
      `${task.id}/folder/${folder.id}`,
  ...
  async updateTaskAssigneeOne(): Promise<void> {
    ...
    expect(taskActionAssigneeResponse).toEqual(
      expect.objectContaining({ userId, taskActionId: commentId }),
    );

    const { body: userActionResponse } = await this.get(
      `/user-actions/folder/${folder.id}/task/${task.id}`,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
    expect(userActionResponse).toHaveLength(1);
    expect(userActionResponse[0].userId).toBe(userId);
    expect(userActionResponse[0].taskActionAssigneeId).toBe(
      taskActionAssigneeResponse.id,
    );
    ...
    const removeTaskAssigneeDto: TaskAssigneesDto = {
      assignees: [],
      folderId: folder.id,
    };
    await this.patch(
  ...
  async updateManyTasks(): Promise<void> {
    ...
    const { body: response1 } = await this.get(
      `${task1.id}/folder/${folder.id}`,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
    const { body: response2 } = await this.get(
  ...
  async updateManyTasksTags(): Promise<void> {
    ...
    this.logger.debug(
      'Request should fail if the personal tag does not belong to the user',
    );
    ...
    const { body: notFoundResponse1 } = await this.patch(
      `many/update`,
      updateFailDto,
      jwtToken.accessToken,
    ).expect(HttpStatus.BAD_REQUEST);
    expect(notFoundResponse1.message).toBe(
      `Tag ( Id: ${user2Tag.id} ) is invalid.`,
    );
    ...
    this.logger.debug(
      'Request should fail if a common tag is not added to the folder first',
    );
    const { body: notFoundResponse2 } = await this.patch(
      `many/update`,
      updateManyTaskDto,
      jwtToken.accessToken,
    ).expect(HttpStatus.NOT_FOUND);
    expect(notFoundResponse2.message).toBe(
      `Tag ( Id: ${tag1.id} ) does not exist in folder ( Id: ${folder.id} ).`,
    );
    ...
    const labelActions = taskActionResponse1.data.filter(
      (el) =>
        el.action === TaskActionOptions.TAG_ADDED ||
        el.action === TaskActionOptions.TAG_REMOVED,
    );
    expect(labelActions).toHaveLength(2);
    expect(
      taskActionResponse1.data.filter(
        (el) => el.action === TaskActionOptions.TAG_ADDED,
      ),
    ).toHaveLength(1); // adding personal tags should not have logs generated
    expect(
      taskActionResponse1.data.filter(
        (el) => el.action === TaskActionOptions.TAG_REMOVED,
      ),
    ).toHaveLength(1);
    ...
    const { body: failResponse } = await this.patch(
      `many/update`,
      failUpdateDto1,
      jwtToken.accessToken,
    ).expect(HttpStatus.NOT_FOUND);
    expect(failResponse.message).toBe(
      `Tag ( Id: ${invalidTagId} ) does not exist in database.`,
    );
    ...
    const { body: failResponse2 } = await this.patch(
      `many/update`,
      failUpdateDto2,
      jwtToken.accessToken,
    ).expect(HttpStatus.NOT_FOUND);
    expect(failResponse2.message).toBe(
      `Tag ( Id: ${tag4.id} ) does not exist in folder ( Id: ${folder.id} ).`,
    );
  }
  ...
  async updateManyTasksCustomFields(): Promise<void> {
    ...
    const { body: spaceDB } = await this.get(
      `/space/${spaceId}`,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
    ...
    const allCFVs = await customFieldValueRepo.find({
      where: {
        customFieldDefinitionId: In([cf1.id, cf2.id]),
        taskId: In([task1.id, task2.id]),
      },
    });
    const taskOneCFOne = allCFVs.find(
      (el) => el.taskId === task1.id && el.customFieldDefinitionId === cf1.id,
    );
    const taskOneCFTwo = allCFVs.find(
      (el) => el.taskId === task1.id && el.customFieldDefinitionId === cf2.id,
    );
    const taskTwoCFOne = allCFVs.find(
      (el) => el.taskId === task2.id && el.customFieldDefinitionId === cf1.id,
    );
    const taskTwoCFTwo = allCFVs.find(
      (el) => el.taskId === task2.id && el.customFieldDefinitionId === cf2.id,
    );
    ...
    await this.patch(
      `/space/${spaceId}`,
      { spaceCustomFields: { insert: [fakeCf3] } },
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
    const { body: spaceDB2 } = await this.get(
      `/space/${spaceId}`,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
    expect(spaceDB2.spaceCustomFields).toHaveLength(3);
    const [cf3] = spaceDB2.spaceCustomFields.filter(
      (el) => el.id !== cf1.id && el.id !== cf2.id,
    );
      ...
      tasks: [
        {
          id: task1.id,
          customFields: [
            { customFieldDefinitionId: fakeId, value: 'Doesnt matter' },
          ],
          folderId: folder.id,
        },
      ],
    ...
    const { body: failResponse1 } = await this.patch(
      `many/update`,
      updateManyTaskDto2,
      jwtToken.accessToken,
    ).expect(HttpStatus.NOT_FOUND);
    expect(failResponse1.message).toBe(
      `Custom Field ( ID: ${fakeId} ) does not exist in database.`,
    );
      ...
      tasks: [
        {
          id: task1.id,
          customFields: [
            { customFieldDefinitionId: cf3.id, value: 'Doesnt matter' },
          ],
          folderId: folder.id,
        },
      ],
    ...
    expect(failResponse2.message).toBe(
      `Custom fields with ID:( ${cf3.id} ) does not exist in the folder: ${folder.id}.`,
    );
  ...
  async createDependencyOnSharedTask(): Promise<void> {
    ...
    const { folder: folder2 } = await this.createFolder(
      null,
      jwtToken,
      workflowDB.id,
      spaceId,
    ...
    const folder2Workflow = await this.folderFactory.getFolderWorkFlow(
      folder2.id,
    );
    const userId = this.getUserIdFromAccessToken(jwtToken.accessToken);

    // 2. Create two tasks in folder 1 and 1 task in folder 2
    const folder1FakeTasks = Array.from({ length: 2 }, () =>
      this.factory.fakeCreateTask(userId, folder1.id),
    );
    ...
    const [
      ...
      ...folder1FakeTasks.map((fakeTask) =>
        this.post(``, fakeTask, jwtToken.accessToken).expect(
          HttpStatus.CREATED,
        ),
      ),
      this.post(``, folder2FakeTask, jwtToken.accessToken).expect(
        HttpStatus.CREATED,
      ),
    ]);
    ...
    const { body: notShareTask } = await this.get(
      `${folder2Task1.id}/folder/${folder2.id}`,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
    const fakeUpdateTask = this.factory.fakeUpdateTask(folder1.id);
    fakeUpdateTask.startDate = moment(notShareTask.endDate)
      .subtract(10, 'days')
      .toDate();
    fakeUpdateTask.endDate = moment(fakeUpdateTask.startDate)
      .add(2, 'days')
      .toDate();
    await this.patch(
      `${folder1Task1.id}`,
    ...
    const { body: taskResponse } = await this.get(
      `${folder1Task1.id}/folder/${folder2.id}`,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
    expect(taskResponse.startDate).toBeDateEqualWithoutMilliseconds(
      notShareTask.endDate,
    );
  }
  ...
  async moveTask(): Promise<void> {
    ...
    this.logger.debug('move task');
    const workFlowEntity = await this.folderFactory.getFolderWorkFlow(
      folder.id,
    );
    const moveTaskDto: UpdateTaskPositionDto = {
      folderId: folder.id,
      actualFolderId: folder.id,
      columnId:
        workFlowEntity.WorkFlowStates[workFlowEntity.WorkFlowStates.length - 1]
          .id,
      index: 1,
      view: FolderViewOptions.BOARD,
    };
    ...
    const exists = response.data.find(
      (x: { action: string; taskId: number }) =>
        x.action === TaskActionOptions.MOVE && x.taskId === task.id,
    );
    ...
    this.logger.debug('get moved task comments');
    const { body: response2 } = await this.post(
      `/task-action/folder/${folder.id}/task/${task.id}`,
      {},
      jwtToken.accessToken,
    ).expect(HttpStatus.CREATED);
    ...
    const workFlowEntity2 = await this.folderFactory.getFolderWorkFlow(
      f2.folder.id,
    );
    const moveTaskDto3: UpdateTaskPositionDto = {
      ...
      actualFolderId: folder.id,
      columnId:
        workFlowEntity2.WorkFlowStates[
          workFlowEntity2.WorkFlowStates.length - 1
        ].id,
      index: 1,
      view: FolderViewOptions.BOARD,
    };
  ...
  async updateWorkflowAdmoveTaskToCreateApproval(): Promise<void> {
    ...
    const { body: wf3 } = await this.patch(
      `/workflow/module/${workflowDB.id}`,
      updatedWorkflow,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
    ...
    const approvalCreatePromise = enqueueEventAndGetPromise(
      ...
      this.approvalApiConnectorService,
      (job, data) =>
        +job?.data?.metaData?.find(
          (m) => m.entityType === ApprovalEntityTypesOptions.TASK,
    ...
    const returnQueue = new Queue(returnQueueName, {
    ...
    this.logger.debug(
      'move task again to replicate reoponing of approval should fail',
    );
    const { body: failedMove } = await this.patch(
      `position/${task.id}`,
      moveTaskDto2,
      jwtToken.accessToken,
    ).expect(HttpStatus.BAD_REQUEST);
    ...
    await this.getDataSource()
      .getRepository(ApprovalConstraintInstanceEntity)
      .update({ taskId: task.id }, { isActive: false });
  ...
  async updatingWorkflowToKeepApprovalConstraintInstance(): Promise<void> {
    ...
    const { body: wf4 } = await this.patch(
      `/workflow/module/${workflowDB.id}`,
      updatedWorkflow2,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
  ...
  async CreateWorkflowAndmoveTaskToCreateApproval(): Promise<void> {
    ...
    const { body: systemStages } = await this.get(
      `/displacement-group/system-stage`,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);

    const fakeWorkflow = await this.workflowFactory.fakeCreateWorkflow(
      systemStages[0]?.id,
    );
    fakeWorkflow.states[0].constraints = [
      {
        ...
        swimlaneConstraint: [
          fakeWorkflow.states[1].code,
          fakeWorkflow.states[2].code,
        ],
      },
    ];
    ...
    const { body: workflowDB } = await this.post(
      `/workflow/module`,
      fakeWorkflow,
      jwtToken.accessToken,
    ).expect(HttpStatus.CREATED);
    this.logger.debug(`check workflow exists`);
    const { body: workflow } = await this.get(
      `/workflow/module/${workflowDB.id}`,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
    ...
    expect(workflow.states[1].approvalConstraint?.acceptState).toBe(
      workflow.states[0].code,
    );
    expect(workflow.states[1].approvalConstraint?.rejectState).toBe(
      workflow.states[2].code,
    );
    expect(workflow.states[1].approvalConstraint?.userIds).toContain(userId1);
    expect(workflow.states[1].approvalConstraint?.authorizedUserIds).toContain(
      userId2,
    );
    ...
    expect(workflow.states[1].approvalConstraint?.dueInType).toBe(
      ApprovalDueInTypeOptions.DAYS,
    );
    ...
    expect(workflow.states[0].constraints[0].swimlaneConstraint).toContain(
      workflow.states[1].code,
    );
    expect(workflow.states[0].constraints[0].swimlaneConstraint).toContain(
      workflow.states[2].code,
    );
    expect(workflow.states[0].constraints[0].swimlaneConstraint).toHaveLength(
      2,
    );
    ...
    const { body: data } = await this.patch(
      `position/${task.id}`,
      moveTaskDto2,
      jwtToken.accessToken,
    ).expect(HttpStatus.BAD_REQUEST);
  ...
  async UpdateWorkflowWithApprovalConstraintState(): Promise<void> {
    ...
    const completed = systemStages.find(
      (s) => s.code === SystemStageCodeOptions.COMPLETED,
    );
    expect(completed).toBeDefined();
    const active = systemStages.find(
      (s) => s.code === SystemStageCodeOptions.ACTIVE,
    );
    expect(active).toBeDefined();
    const canceled = systemStages.find(
      (s) => s.code === SystemStageCodeOptions.CANCELED,
    );
    expect(canceled).toBeDefined();
    const deferred = systemStages.find(
      (s) => s.code === SystemStageCodeOptions.DEFERRED,
    );
    ...
    const fakeUpdateWorkflow =
      this.workflowFactory.fakeUpdateWorkflow(workflow);

    fakeUpdateWorkflow.states = fakeUpdateWorkflow.states.map((s) => ({
      ...s,
      updated: false,
    }));
    ...
    fakeUpdateWorkflow.states = fakeUpdateWorkflow.states.map((s, i) => ({
      ...s,
      index: i,
    }));
    ...
    const { body: workflowUpdated } = await this.get(
      `/workflow/module/${workflowDB.id}`,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
    this.logger.debug(`check in detail workflow approval constraint`);
    ...
    expect(workflowUpdated.states[2].title).toBe(
      fakeUpdateWorkflow.states[2].title,
    );
    ...
    const moveTaskDto: UpdateTaskPositionDto = {
      folderId: f1.id,
      actualFolderId: f1.id,
      columnId: workflowUpdated.states.find((s) => s.title === newState.title)
        .id,
      index: 0,
      view: FolderViewOptions.BOARD,
    };
    ...
    const fakeUpdateWorkflow2 =
      this.workflowFactory.fakeUpdateWorkflow(workflowUpdated);

    fakeUpdateWorkflow2.states = fakeUpdateWorkflow2.states.map((s) => ({
      ...s,
      updated: false,
    }));
    ...
    fakeUpdateWorkflow2.states = fakeUpdateWorkflow2.states.map((s, i) => ({
      ...s,
      index: i,
    }));
    const { body: response2 } = await this.patch(
    ...
    const { body: workflowUpdated2 } = await this.get(
      `/workflow/module/${workflowDB.id}`,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
    this.logger.debug(`check in detail workflow approval constraint`);
  ...
  async shareTask(): Promise<void> {
    ...
    this.logger.debug('get moved task comments');
    const { body: response } = await this.post(
      `/task-action/folder/${folder1.id}/task/${task.id}`,
      {},
      jwtToken.accessToken,
    ).expect(HttpStatus.CREATED);
    const exists = response.data.find(
      (x: { action: string; taskId: number }) =>
        x.action === TaskActionOptions.SHARED && x.taskId === task.id,
    );
    expect(exists.parameters?.share).toMatchObject(shareTaskDto);

    this.logger.debug('un-share task');
    await this.delete(
      `un-share/${task.id}/${folder2.id}`,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
    this.logger.debug('get moved task comments');
    const { body: response2 } = await this.post(
    ...
    const exists2 = response2.data.find(
      (x: { action: string; taskId: number }) =>
        x.action === TaskActionOptions.UNSHARED && x.taskId === task.id,
    );
    expect(exists2.parameters?.unshared).toMatchObject({
  ...
  async moveSharedTasksToNewFolder(): Promise<void> {
    ...
    const { folder: folder3 } = await this.createFolder(
      null,
      jwtToken,
      workflowDB.id,
      spaceId,
    ); //sharing should be done in same space
    const userId = this.getUserIdFromAccessToken(jwtToken.accessToken);
    ...
    const { body: board2 } = await this.post(
      `/folder-workflow/project/${folder2.id}/board`,
      {},
      jwtToken.accessToken,
    ).expect(HttpStatus.CREATED);
    ...
    const folder2WorkFlowEntity = await this.folderFactory.getFolderWorkFlow(
      folder2.id,
    );

    const folder3WorkFlowEntity = await this.folderFactory.getFolderWorkFlow(
      folder3.id,
    );
    ...
    for (let i = 0; i < folder2WorkFlowEntity.WorkFlowStates.length; i++) {
      mapWorkflowStates.push({
        ...
        destinationWorkflowStateCode:
          folder3WorkFlowEntity.WorkFlowStates[i]?.code ??
          folder3WorkFlowEntity.WorkFlowStates[0]?.code,
      });
    }
    ...
    await this.put(`move-many`, moveTaskDto, jwtToken.accessToken).expect(
      HttpStatus.OK,
    );

    const { body: board2Empty } = await this.post(
      `/folder-workflow/project/${folder2.id}/board`,
      {},
      jwtToken.accessToken,
    ).expect(HttpStatus.CREATED);
    ...
    const { body: board3 } = await this.post(
      `/folder-workflow/project/${folder3.id}/board`,
      {},
      jwtToken.accessToken,
    ).expect(HttpStatus.CREATED);
  ...
  async moveOneTaskToNewFolder(): Promise<void> {
    ...
    await this.patch(
      ...
      {
        tags: { insert: [tagSpace1.id] },
        customFieldValues: {
          insert: [{ id: customFieldIdsSpace1[0], value: '' }],
        },
      },
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
    ...
    await this.patch(
      ...
      {
        tags: { insert: [tagSpace2.id] },
        customFieldValues: {
          insert: [{ id: customFieldIdsSpace2[0], value: '' }],
        },
      },
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
    ...
    const { body: subtask1 } = await this.post(
      ...
      fakeSubTask1,
      jwtToken.accessToken,
    ).expect(HttpStatus.CREATED); //this generates an update on task1 so +1 in task action expected

    this.logger.debug('add tag to task');
    await this.post(
      `tag/${task1.id}/${tagSpace1.id}/${folder1.id}`,
      {},
      jwtToken.accessToken,
    ).expect(HttpStatus.CREATED);
    const commentDto: CreateTaskActionDto = {
    ...
    await this.post(
      `/task-action/folder/${folder1.id}/task/${task1.id}/comment`,
      commentDto,
      jwtToken.accessToken,
    ).expect(HttpStatus.CREATED);
    ...
    const addApprovalToTaskDto =
      this.approvalsFactory.fakeCreateAddApprovalToTaskDto([userId3]);
    const { body: createdApproval } = await this.post(
    ...
    const addApprovalToTaskDto2 =
      this.approvalsFactory.fakeCreateAddApprovalToTaskDto([userId4]);
    ...
    const moveTaskDto: MoveOneTaskDto = {
      ...
      mappedCustomFields: [
        { oldId: customFieldIdsSpace1[0], newId: customFieldIdsSpace2[0] },
      ],
    ...
    const { body: board1Empty } = await this.post(
      `/folder-workflow/project/${folder1.id}/board`,
      {},
      jwtToken.accessToken,
    ).expect(HttpStatus.CREATED);
    ...
    const { body: folder2DB } = await this.get(
      `/folder/${folder2.id}?show-on=${TASK_MANAGEMENT}`,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
    ...
    const { body: approvalsTask1 } = await this.get(
      `/approvals/folder/${folder2.id}/task/${task1.id}`,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
    const approvalTask1 = approvalsTask1.find(
      (el) => el.id === createdApproval.id,
    );
    ...
    const { body: task1CustomFields } = await this.get(
      `custom-field/folder/${folder2.id}/task/${task1.id}`,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
    expect(task1CustomFields.customFields[0].customFieldDefinitionId).toBe(
      customFieldIdsSpace2[0],
    );
    expect(task1CustomFields.customFields[0].value).toBe(customField1Value);

    const { body: task1Folder2 } = await this.get(
      `${task1.id}/folder/${folder2.id}?show-all-children=true`,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
    ...
    const approvalSubtask1 = approvalsSubtask1.find(
      (el) => el.id === createdApproval2.id,
    );
    ...
    const { body: taskAction } = await this.post(
      `/task-action/folder/${folder2.id}/task/${task1.id}`,
      {},
      jwtToken.accessToken,
    ).expect(HttpStatus.CREATED);
    expect(taskAction).toBeDefined();
    ...
    const created = taskAction.data.find(
      (x: { action: string; taskId: number }) =>
        x.action === TaskActionOptions.CREATE && x.taskId === task1.id,
    );
    expect(created).toBeDefined();
    const taskFound = created.parameters?.create;
    ...
    const moved = taskAction.data.find(
      (x: { action: string; taskId: number }) =>
        x.action === TaskActionOptions.TASK_FOLDER_MOVE &&
        x.taskId === task1.id,
    );
    ...
    const comment = taskAction.data.find(
      (x: { action: string; taskId: number }) =>
        x.action === TaskActionOptions.COMMENT && x.taskId === task1.id,
    );
  ...
  async shareTaskBetweenParentChildFolder(): Promise<void> {
    ...
    const { body: board1 } = await this.post(
      `/folder-workflow/project/${folder2.id}/board`,
      {},
      jwtToken.accessToken,
    ).expect(HttpStatus.CREATED);
    ...
    const { body: sharedTask } = await this.post(
      `share/${task.id}`,
      shareTaskDto,
      jwtToken.accessToken,
    ).expect(HttpStatus.CREATED);
    ...
    const taskFolder1 = column2.tasks.find(
      (t) => t.id === task.id && t.folderId === folder1.id,
    );
    expect(taskFolder1.id).toBe(task.id);
    const taskFolder2 = column2.tasks.find(
      (t) => t.id === task.id && t.folderId === folder2.id,
    );
  ...
  async moveBetweenFolders(): Promise<void> {
    ...
    const folder1WorkFlowEntity = await this.folderFactory.getFolderWorkFlow(
      folder1.id,
    );
    ...
    for (let i = 0; i < folder1WorkFlowEntity.WorkFlowStates.length; i++) {
      mapWorkflowStates.push({
        ...
        destinationWorkflowStateCode:
          folder2WorkFlowEntity.WorkFlowStates[i]?.code ??
          folder2WorkFlowEntity.WorkFlowStates[0]?.code,
      });
    }
    ...
    const parentTask = folderTaskTree2[0].columns.find((column) =>
      column.tasks.find((task) => task.id === task1.id),
    );
    ...
    this.logger.debug(
      'move more than 1000 tasks at once, should fail with Bad request',
    );
    ...
    await this.put(`move-many`, moveTaskDtoToFail, jwtToken.accessToken).expect(
      HttpStatus.BAD_REQUEST,
    );
  }
  ...
  async CheckTaskUpdatePathTitle(): Promise<void> {
    ...
    const { body: taskTreeResponse } = await this.get(
      `/folder/task-tree/${folder.id}`,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
  ...
  async moveTaskPosition(): Promise<void> {
    ...
    const { body: parentTask } = await this.get(
      `${task1.id}/folder/${folder.id}`,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
    expect(parentTask.id).toBe(task1.id);
    const foundChild = parentTask.children.find(
      (child) => child.id === task2.id,
    );
  ...
  async updateTaskPathIds(): Promise<void> {
    ...
    const fakeTasks = Array.from({ length: 4 }, () =>
      this.factory.fakeCreateTask(userId, folder.id),
    );
    ...
    const { body: task1Updated } = await this.get(
      `${task1.id}/folder/${folderId}`,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
    const { body: task2Updated } = await this.get(
      `${task2.id}/folder/${folderId}`,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
    const { body: task3Updated } = await this.get(
      `${task3.id}/folder/${folderId}`,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
    ...
    expect(taskWithChildren.children[0].children[0].pathIds).toEqual([
      task1.id,
      task2.id,
      task3.id,
    ]);
    ...
    expect(taskWithChildren.children[0].children[0].children[0].id).toBe(
      task4.id,
    );
    expect(
      taskWithChildren.children[0].children[0].children[0].pathIds,
    ).toEqual([task1.id, task2.id, task3.id, task4.id]);
  }
  ...
  async taskPathIdsFilteredByFolder(): Promise<void> {
    ...
    const folder1Workflow = await this.folderFactory.getFolderWorkFlow(
      folder1.id,
    );
    const folder2Workflow = await this.folderFactory.getFolderWorkFlow(
      folder2.id,
    ...
    const { body: folder1Task1 } = await this.post(
      ``,
      folder1FakeTasks[0],
      jwtToken.accessToken,
    ).expect(HttpStatus.CREATED);
    const { body: folder1Task2 } = await this.post(
      ``,
      folder1FakeTasks[1],
      jwtToken.accessToken,
    ).expect(HttpStatus.CREATED);
    ...
    await this.patch(
      `position/${folder1Task2.id}`,
      folder1SubtaskDto,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);

    // 4. Create two tasks in folder 2
    const folder2FakeTasks = Array.from({ length: 2 }, () =>
      this.factory.fakeCreateTask(userId, folder2.id),
    );
    this.logger.debug('create tasks');
    //separate this , cause can create deadlock
    const { body: folder2Task1 } = await this.post(
      ``,
      folder2FakeTasks[0],
      jwtToken.accessToken,
    ).expect(HttpStatus.CREATED);
    const { body: folder2Task2 } = await this.post(
      ``,
      folder2FakeTasks[1],
      jwtToken.accessToken,
    ).expect(HttpStatus.CREATED);
    ...
    const { body: shareResult } = await this.post(
      `share/${folder1Task1.id}`,
      shareDto,
      jwtToken.accessToken,
    ).expect(HttpStatus.CREATED);
    ...
    await this.patch(
      `position/${shareResult.childTaskId}`,
      folder2SharedSubtaskDto,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
    ...
    const f1Column = f1Columns.find(
      ({ id }) => id === folder1Workflow.WorkFlowStates[0].id,
    );
    ...
    // Folder 1 - A2
    const { body: f1Task } = await this.get(
      `${f1Tasks[0].id}/folder/${folder1.id}?show-all-children=true`,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
    expect(f1Task.children.length).toBe(1);
    expect(f1Task.children[0].id).toBe(folder1Task2.id);
    expect(f1Task.children[0].pathIds).toEqual([
      folder1Task1.id,
      folder1Task2.id,
    ]);
    ...
    const f2Column = f2Columns.find(
      ({ id }) => id === folder2Workflow.WorkFlowStates[0].id,
    );
    ...
    // Folder 2 - B2
    const { body: f2Task } = await this.get(
      `${f2Tasks[0].id}/folder/${folder2.id}?show-all-children=true`,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
    expect(f2Task.children.length).toBe(1);
    expect(f2Task.children[0].id).toBe(folder2Task2.id);
    expect(f2Task.children[0].pathIds).toEqual([
      folder2Task1.id,
      folder2Task2.id,
    ]);
    ...
    expect(f2Task.children[0].children[0].pathIds).toEqual([
      folder2Task1.id,
      folder2Task2.id,
      folder1Task1.id,
    ]);
    ...
    await this.post(
      `archive/${folder2Task2.id}/folder/${folder2.id}`,
      { archiveReason: 'for test' },
      jwtToken.accessToken,
    ).expect(HttpStatus.CREATED);
    ...
    this.logger.debug(
      'Archived tasks should be shown in the task list if set show archived children to true',
    );
  ...
  async unshareSubTaskShouldRemoveExistingRelation(): Promise<void> {
    ...
    await this.post(
      `share/${folder1Task2.id}`,
      shareDto,
      jwtToken.accessToken,
    ).expect(HttpStatus.CREATED);

    this.logger.debug('Task 2 relation should remain in folder 1');
    const checkTaskTwoRelationExist1 = await this.factory.dataSource
      .getRepository(TaskRelationEntity)
      .existsBy({
  ...
  async subtaskShouldNotBeDuplicated(): Promise<void> {
    ...
    this.logger.debug(
      'Shared task1 in folders should only contain one subtask',
    );
    const { body: taskResponse } = await this.get(
      `${folder1Task1.id}/folder/${folder1.id}`,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
  ...
  async changeTaskParent(): Promise<void> {
    ...
    const { body: parentTask1 } = await this.post(
      ``,
      fakeParentTask1,
      jwtToken.accessToken,
    ).expect(HttpStatus.CREATED);
    const fakeParentTask2 = this.factory.fakeCreateTask(userId, folder.id);
    const { body: parentTask2 } = await this.post(
      ``,
      fakeParentTask2,
      jwtToken.accessToken,
    ).expect(HttpStatus.CREATED);
    ...
    const { body: parentTask } = await this.get(
      `${parentTask2.id}/folder/${folder.id}`,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
    ...
    const { body: rootTask } = await this.get(
      `${task2.id}/folder/${folder.id}`,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
  ...
  async moveSharedSubtaskToInvalidColumn(): Promise<void> {
    ...
    // 2. Create folder 1 with workflow 1
    const { folder: folder1, spaceId } = await this.createFolder(
      null,
      jwtToken,
      workflow1.id,
    );
    ...
    await this.patch(
      `/space/${spaceId}`,
      { moduleWorkflows: { insert: [workflow2.id], delete: [] } },
      jwtToken.accessToken,
    );
    ...
    const { body: workflows } = await this.post(
      `/folder-workflow/project`,
      [folder1.id, folder2.id],
      jwtToken.accessToken,
    ).expect(HttpStatus.CREATED);

    const folder1Workflow: ProjectWorkFlowResponseDto = workflows.find(
      ({ folderId }) => folderId === folder1.id,
    );
    const folder2Workflow: ProjectWorkFlowResponseDto = workflows.find(
      ({ folderId }) => folderId === folder2.id,
    );
    ...
    // 4. Create 2 tasks in folder 1
    const fakeTasks = Array.from({ length: 2 }, () =>
      this.factory.fakeCreateTask(userId, folder1.id),
    );
    this.logger.debug('create task');
    const [{ body: task1 }, { body: task2 }] = await Promise.all(
      fakeTasks.map((fakeTask) =>
        this.post(``, fakeTask, jwtToken.accessToken).expect(
          HttpStatus.CREATED,
        ),
      ),
  ...
  async convertSubtaskToTask(): Promise<void> {
    ...
    const fakeTasks = Array.from({ length: 2 }, () =>
      this.factory.fakeCreateTask(userId, subFolder.id),
    );
    this.logger.debug('create task');
    const [{ body: task1 }, { body: task2 }] = await Promise.all(
      fakeTasks.map((fakeTask) =>
    ...
    const folderWorkflow: ProjectWorkFlowResponseDto = workflows.find(
      ({ folderId }) => folderId === subFolder.id,
    );
    ...
    const { body: subtask } = await this.patch(
      `position/${task2.id}`,
      createSubtaskDto,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
    ...
    const { body: convertedTask } = await this.patch(
      `position/${task2.id}`,
      convertToTaskDto,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
  ...
  async testTaskAutomationResponse(): Promise<void> {
    ...
    const { body: taskResponse } = await this.post(
      `automationstmduetasks`,
      { folderId: folder.id },
      jwtToken.accessToken,
    ).expect(HttpStatus.CREATED);
  ...
  async attachmentUrlMustChange(): Promise<void> {
    ...
    const attachments1 = await this.get(
      `/task-attachment/folder/${folder.id}/task/${task.id}`,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
    ...
    const attachments2 = await this.get(
      `/task-attachment/folder/${folder.id}/task/${task.id}`,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
    ...
    expect(attachments1.body[0].fileNameUrl).not.toBe(
      attachments2.body[0].fileNameUrl,
    );
    expect(attachments1.body[0].thumbnailUrl).not.toBe(
      attachments2.body[0].thumbnailUrl,
    );
  }

  // @Test('Custom Fields Collection Attach/Delete on Space,Folder,TaskEntity')
  ...
  async createCustomFieldPositionOnTask(): Promise<void> {
    ...
    const { body: allCustomFields } = await this.get(
      `custom-field/folder/${folder.id}/task/${task.id}`,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
    ...
    expect(commonCustomFields[0].customFieldDefinitionId).toBe(
      customFieldIds[3],
    );
    expect(commonCustomFields[0].index).toBe(0);

    expect(commonCustomFields[1].customFieldDefinitionId).toBe(
      customFieldIds[2],
    );
    expect(commonCustomFields[1].index).toBe(1);

    expect(commonCustomFields[4].customFieldDefinitionId).toBe(
      customFieldIds[1],
    );
    expect(commonCustomFields[4].index).toBe(4);

    expect(commonCustomFields[6].customFieldDefinitionId).toBe(
      customFieldIds[0],
    );
    ...
    expect(userCustomFields[2].customFieldDefinitionId).toBe(
      userCustomFieldIds[3],
    );
    expect(userCustomFields[2].index).toBe(2);

    expect(userCustomFields[3].customFieldDefinitionId).toBe(
      userCustomFieldIds[2],
    );
    expect(userCustomFields[3].index).toBe(3);

    expect(userCustomFields[5].customFieldDefinitionId).toBe(
      userCustomFieldIds[1],
    );
    expect(userCustomFields[5].index).toBe(5);

    expect(userCustomFields[7].customFieldDefinitionId).toBe(
      userCustomFieldIds[0],
    );
  ...
  async replicateTask(): Promise<void> {
    ...
    this.logger.debug(
      'create space1 with two members, two tags and two custom field definitions',
    );
    const members = [
    ...
    const { body: spaceResponse } = await this.post(
      `/space`,
      fakeCreateSpace,
      token.accessToken,
    ).expect(HttpStatus.CREATED);
    ...
    const { body: folder1 } = await this.post(
      `/folder`,
      fakeFolder,
      token.accessToken,
    ).expect(HttpStatus.CREATED);
    ...
    this.logger.debug(
      'create space2 with 1 co-exist member, tag and custom field',
    );
    const space2members = [
      { id: user1Id, userPermission: UserPermissionOptions.FULL },
    ];
    ...
    const { body: space2Response } = await this.post(
      `/space`,
      fakeCreateSpace2,
      token.accessToken,
    ).expect(HttpStatus.CREATED);
    ...
    const { body: folder2 } = await this.post(
      `/folder`,
      fakeFolder2,
      token.accessToken,
    ).expect(HttpStatus.CREATED);
    ...
    const { body: taskResponse2 } = await this.post(
      ``,
      fakeTask2,
      token.accessToken,
    ).expect(HttpStatus.CREATED);
    ...
    const { body: subTaskResponse2 } = await this.post(
      ``,
      fakeSubTask2,
      token.accessToken,
    ).expect(HttpStatus.CREATED);
    ...
    this.logger.debug(
      'create one task in folder 1 with two tags and a sub-task, assign user1 and user2 to the task',
    );
    ...
    const fakeSubTask = this.factory.fakeCreateTask(
      creatorId,
      folder1.id,
      taskResponse.id,
    );
    const { body: subTaskResponse } = await this.post(
      ``,
      fakeSubTask,
      token.accessToken,
    ).expect(HttpStatus.CREATED);
    ...
    const fakeFiles = await this.taskAttachmentFactory.fakeFiles();
    await this.post(
      `/task-attachment/upload/${taskResponse.id}/folder/${folder1.id}`,
      undefined,
      token.accessToken,
      fakeFiles,
    ).expect(HttpStatus.CREATED);
    ...
    this.logger.debug(
      'create a comment with mentioning two members and an attachment',
    );
    const commentDto: CreateTaskActionDto = {
    ...
    expect(uploadResponse[0].TaskAction.id).toBe(
      commentResponse.identifiers[0].id,
    );
    ...
    const { body: taskDb1 } = await this.get(
      `${taskResponse.id}/folder/${folder1.id}`,
      token.accessToken,
    ).expect(HttpStatus.OK);
    ...
    expect(taskDb1.followers).toEqual(
      expect.arrayContaining([creatorId, user1Id, user2Id]),
    );
    expect(taskDb1.assignees).toHaveLength(2);
    expect(taskDb1.assignees).toEqual(
      expect.arrayContaining([user1Id, user2Id]),
    );
    expect(taskDb1.tags).toHaveLength(2);
    expect(taskDb1.tags).toEqual(
      expect.arrayContaining([tagDb1.id, tagDb2.id]),
    );

    const { body: taskCFDb } = await this.get(
      `custom-field/folder/${folder1.id}/task/${taskDb1.id}`,
      token.accessToken,
    ).expect(HttpStatus.OK);
    expect(taskCFDb.customFields).toHaveLength(2);
    expect(
      taskCFDb.customFields.map((cf) => cf.customFieldDefinitionId),
    ).toEqual(expect.arrayContaining([CF1.id, CF2.id]));
    ...
    const { Actions: taskActions } = await taskRepo.findOne({
      where: {
        ...
        Actions: {
          action: In([
            TaskActionOptions.COMMENT,
            TaskActionOptions.COMMENT_REACTION,
          ]),
        },
      },
    ...
    expect(taskActions[0].likes).toEqual([
    ...
    const taskAttachmentDb = await taskAttachementRepo.find({
      where: { taskId: taskResponse.id },
    });
    ...
    await this.post(
      `replicate/${taskDb1.id}`,
      replicateTaskFailDto,
      token.accessToken,
    )
      .expect(HttpStatus.NOT_FOUND)
      .catch((e) =>
        expect(e.message).toBe(
          `Task with id ${taskDb1.id} found in Gantt Model for folder ${replicateTaskFailDto.origin.folderId}`,
        ),
      );
    ...
    const { body: duplicateTaskResponse } = await this.post(
      `replicate/${taskDb1.id}`,
      replicateTaskDto,
      token.accessToken,
    ).expect(HttpStatus.CREATED);
    ...
    this.logger.debug(
      'Detail for task should be identical except for id and createdAt',
    );
    ...
    const { body: clonedTaskDB } = await this.get(
      `${duplicateTaskResponse.id}/folder/${folder2.id}`,
      token.accessToken,
    ).expect(HttpStatus.OK);
    ...
    this.logger.debug(
      'User 2 should not be in the followers and assigness as he has no access to space2 or folder2',
    );
    expect(clonedTaskDB.followers).toHaveLength(2);
    expect(clonedTaskDB.followers).toEqual(
      expect.arrayContaining([creatorId, user1Id]),
    );
    ...
    const clonedTagsDB = await this.dataSource
      .getRepository(TagEntity)
      .find({ where: { id: In(clonedTaskDB.tags) } });
    ...
    const clonedCF1 = clonedTaskCFdb.customFields.find(
      (cf) => cf.CustomFieldDefinition.title === CF1.title,
    );
    expect(clonedCF1.id).not.toBe(CF1.id);
    const clonedCF2 = clonedTaskCFdb.customFields.find(
      (cf) => cf.CustomFieldDefinition.title === CF2.title,
    );
    expect(clonedCF2.id).not.toBe(CF2.id);

    this.logger.debug(
      'Comment should be cloned and only User1 should remain in the mentioned and liked array',
    );
    ...
    expect(Actions[0].likes).toEqual([
      expect.objectContaining({ userId: user1Id }),
    ]);
    ...
    const clonedTaskAttachmentDb = await taskAttachementRepo.find({
      where: { taskId: duplicateTaskResponse.id },
    });
    expect(clonedTaskAttachmentDb).toHaveLength(1);
    expect(clonedTaskAttachmentDb[0].originalName).toBe(
      taskAttachmentDb[0].originalName,
    );
    expect(clonedTaskAttachmentDb[0].fileName).not.toBe(
      taskAttachmentDb[0].fileName,
    );
    ...
    const { body: clonedTask2 } = await this.post(
      `replicate/${taskDb1.id}`,
      replicateTaskDto2,
      token.accessToken,
    ).expect(HttpStatus.CREATED);
  ...
  async moveTaskToAnotherSpace(): Promise<void> {
    const repoCustomField = this.dataSource.getRepository(
        CustomFieldDefinitionEntity,
      ),
      repoCFVposition = this.dataSource.getRepository(
        CustomFieldValuePositionEntity,
      );
    this.logger.debug('create users');
    const { token } = await this.userSuite.createSuperUser();
    ...
    this.logger.debug('create folder1 in space 1 with two members');
    const fakeFolder = this.folderFactory.fakeCreateFolderV2(
      workflow.id,
      spaceResponse.id,
      { members, tags: [tagDb1.id] },
    );
    const { body: folder1 } = await this.post(
      `/folder`,
      fakeFolder,
    ...
    this.logger.debug(
      'create space2 with 1 co-exist member, 1 co-exsit and 1 new tags and 1 co-exist and 1 new custom fields',
    );
    const fakeTag3 = this.tagFactory.fakeCreateTag(); //To be inherit to task after moved
    const fakeCf3 =
      this.customFieldDefinitionFactory.fakeCreateCustomFieldDefinition(); //To be inherit to task after moved
    const space2members = [
      { id: user1Id, userPermission: UserPermissionOptions.FULL },
    ];
    const fakeCreateSpace2 = this.folderFactory.fakeCreateSpace({
    ...
    const CF3 = space2DB.spaceCustomFields.find(
      (el) => el.title === fakeCf3.title,
    );
    expect(CF3).toBeDefined();

    this.logger.debug(
      'create folder2 in space 2 with 1 member, using workflow2',
    );
    const fakeFolder2 = this.folderFactory.fakeCreateFolderV2(
      workflow2.id,
      space2Response.id,
      {
        members: space2members,
        tags: space2DB.spaceTags.map((el) => el.id),
        customFieldValues: space2DB.spaceCustomFields.map((el) => ({
          id: el.id,
          value: null,
        })),
      },
    ...
    const fakeTask = this.factory.fakeCreateTaskV2(creatorId, folder1.id, {
      tagsId: [tagDb1.id, tagDb2.id],
      prominentTagId: tagDb1.id,
    });
    fakeTask.assignees = [user1Id, user2Id];
    const { body: taskResponse } = await this.post(
      ``,
    ...
    const updateCFpositionDto = {
      customFieldId: CF1.id,
      folderId: folder1.id,
      index: 0,
    ...
    const cfvPositions = await repoCFVposition.find({
      where: {
        CustomFieldValue: { customFieldDefinitionId: In([CF1.id, CF2.id]) },
      },
    ...
    await this.post(
      `move-to-space/${taskResponse.id}`,
      moveTaskDto,
      token.accessToken,
    ).expect(HttpStatus.CREATED);

    this.logger.debug('Verify update');
    const { body: boardResponse } = await this.post(
      `/folder-workflow/project/${folder2.id}/board`,
      {},
      token.accessToken,
    ).expect(HttpStatus.CREATED);
    ...
    expect(taskMovedResponse.assignees).toEqual(
      expect.arrayContaining([user1Id, user2Id]),
    );
    expect(taskMovedResponse.tags).toHaveLength(2);
    expect(taskMovedResponse.customFields).toHaveLength(3);
    const [clonedCfValue1, clonedCfValue2, clonedCfValue3] =
      taskMovedResponse.customFields;
    ...
    const cloneCfDb = cloneCfDbs.find(
      (el) => el.title === CF1.title && el.type === CF1.type,
    );
    const cloneCf2Db = cloneCfDbs.find(
      (el) => el.title === CF2.title && el.type === CF2.type,
    );
    const cloneCf3Db = cloneCfDbs.find(
      (el) => el.title === CF3.title && el.type === CF3.type,
    );
    ...
    const cfp1Updated = cfpUpdated.find(
      (el) => el.CustomFieldValue.customFieldDefinitionId === cloneCfDb.id,
    );
    const cfp2Updated = cfpUpdated.find(
      (el) => el.CustomFieldValue.customFieldDefinitionId === cloneCf2Db.id,
    );
    ...
    const { body: taskDb2 } = await this.get(
      `${taskResponse.id}/folder/${folder2.id}`,
      token.accessToken,
    ).expect(HttpStatus.OK);
    ...
    const { body: space2Response2 } = await this.get(
      `/space/${space2Response.id}`,
      token.accessToken,
    ).expect(HttpStatus.OK);
    ...
    const { body: taskDb3 } = await this.get(
      `${taskResponse.id}/folder/${folder1.id}`,
      token.accessToken,
    ).expect(HttpStatus.OK);
    ...
    expect(taskDb3.assignees).toEqual(
      expect.arrayContaining([user1Id, user2Id]),
    );
    expect(taskDb3.tags).toHaveLength(0);

    const { body: taskCFDb2 } = await this.get(
      `custom-field/folder/${folder1.id}/task/${taskDb1.id}`,
      token.accessToken,
    ).expect(HttpStatus.OK);
    ...
    await this.post(
      `move-to-space/${subTaskResponse.id}`,
      moveTaskDto3,
      token.accessToken,
    ).expect(HttpStatus.CREATED);

    this.logger.debug(
      'Verify update, sub task should be moved to the root after moved',
    );
    const { body: boardResponse2 } = await this.post(
      `/folder-workflow/project/${folder2.id}/board`,
      {},
      token.accessToken,
    ).expect(HttpStatus.CREATED);
    ...
    this.logger.debug(
      'Moving task to the same folder should succeed without changes',
    );
    ...
    const { body: boardResponse3 } = await this.post(
      `/folder-workflow/project/${folder2.id}/board`,
      {},
      token.accessToken,
    ).expect(HttpStatus.CREATED);
  ...
  async moveTaskToAnotherFolderSameSpace(): Promise<void> {
    ...
    const { body: customField1 } = await this.post(
      `/custom-field-definition/${spaceId}`,
      fakeCf1,
      jwtToken.accessToken,
    ).expect(HttpStatus.CREATED);
    ...
    const { body: spaceDb1 } = await this.get(
      `/space/${spaceId}`,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
    ...
    const { body: folderDb1 } = await this.get(
      `/folder/${folder.id}`,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
    ...
    const { body: taskDb2CF } = await this.get(
      `custom-field/folder/${folder2.id}/task/${task.id}`,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
    expect(taskDb2CF.customFields).toHaveLength(1);
    expect(taskDb2CF.customFields[0].customFieldDefinitionId).toBe(
      customField1.identifiers[0].id,
    );

    const { body: folderDb2AfterMove } = await this.get(
      `/folder/${folder2.id}`,
      jwtToken.accessToken,
    ).expect(HttpStatus.OK);
  ...
  @Test('Move TaskEntity to different Spaces')
  async moveTasksToDifferentSpaces(): Promise<void> {
    ...
    this.logger.debug(
      'create 3 spaces for user 1, 1 for user 2 with user 1 as member',
    );
    const { body: space1 } = await this.post(
      `/space`,
      fakeCreateSpace,
      token.accessToken,
    ).expect(HttpStatus.CREATED);
    const { body: space2 } = await this.post(
      `/space`,
      fakeCreateSpace,
      token.accessToken,
    ).expect(HttpStatus.CREATED);
    const { body: space3 } = await this.post(
      `/space`,
      fakeCreateSpace,
      token.accessToken,
    ).expect(HttpStatus.CREATED);

    fakeCreateSpace.members = [
      { id: user1Id, userPermission: UserPermissionOptions.FULL },
    ];
    const { body: space4 } = await this.post(
      `/space`,
      fakeCreateSpace,
      token2.accessToken,
    ).expect(HttpStatus.CREATED);
    ...
    for (const spaceId of [space1.id, space2.id, space3.id]) {
      ...
      const { body: folder } = await this.post(
        `/folder`,
        fakeFolder,
        accessToken,
      ).expect(HttpStatus.CREATED);
      ...
      const { body: task } = await this.post(``, fakeTask, accessToken).expect(
        HttpStatus.CREATED,
      );
    ...
    const { body: folder4 } = await this.post(
      `/folder`,
      fakeFolder,
      token2.accessToken,
    ).expect(HttpStatus.CREATED);
    ...
    this.logger.debug(
      'Request should fail if user does not have permission to space or folder',
    );
    const { body: failReponse } = await this.post(
      'move-many-to-space',
      moveManyDto,
      token2.accessToken,
    ).expect(HttpStatus.FORBIDDEN);
    ...
    expect(failReponse2.message).toBe(
      `You do not have neccessary permission for the following folders: [ ${folder4.id} ]`,
    );

    this.logger.debug(
      'Request should succeed if user has all necessary permissions',
    );
    await this.post(
      'move-many-to-space',
      moveManyDto,
  ...
  async actionsByServiceUser(): Promise<void> {
    ...
    expect(foundTaskLabelAction.parameters).toEqual(
      expect.objectContaining({ tag: { id: tag.id } }),
    );
    ...
    expect(foundTaskLabelAction2.parameters).toEqual(
      expect.objectContaining({ tag: { id: tag.id } }),
    );
    ...
    const foundTaskCFAction = await this.dataSource
      ...
      .findOneBy({
        taskId: task.id,
        action: TaskActionOptions.CUSTOMFIELD_UPDATED,
      });
  ...
  async createWorkflowWithApprovalsConstraintAndCreateNewTask(): Promise<void> {
    ...
    const spaceResponse = await this.createSpace(jwtToken.accessToken, [
      workflowDb.id,
    ]);
    this.logger.debug('create folder');
    const fakeFolder = this.folderFactory.fakeCreateFolder(
  ...
  async moveTaskToRoot(): Promise<void> {
    ...
    fakeTask.assignees = [userId];
    const { body: task0 } = await this.post(
      ``,
      fakeTask,
      jwtToken.accessToken,
    ).expect(HttpStatus.CREATED);
    ...
    createdTags.forEach((tag) =>
      expect(folderTasks.some((task) => task.id === tag.id)).toBeTruthy(),
    );
    ...
    expect(folderTasks[0].children[0].children[0].children[0].id).toBe(
      task3.id,
    );
    expect(
      folderTasks[0].children[0].children[0].children[0].children.length,
    ).toBe(1);
    expect(
      folderTasks[0].children[0].children[0].children[0].children[0].id,
    ).toBe(task4.id);
    ...
    const folderTasksChildrenIds0 = folderTasks.find(
      (task) => task.id === task0.id,
    );
    const folderTasksChildrenIds1 = folderTasks.find(
  ...
  async createManyCustomFields(
    ...
    for (let i = 1; i <= number; i++) {
      ...
      const foundCF = allCustomFields.filter(
        (cf) => cf.id === customField.identifiers[0].id,
      );
  ...
  private async createWorkflowForFolder(
    token: string,
    ...
    const { body, status } = await this.post(
      `/workflow/module`,
      fakeWorkflow,
      token,
    ).expect(HttpStatus.CREATED);
    ...
    this.logger.debug(`check workflow exists`);
    const { body: workflowDB, status: workflowDBStatus } = await this.get(
      `/workflow/module/${body.id}`,
      token,
    ).expect(HttpStatus.OK);
    expect(workflowDBStatus).toBe(HttpStatus.OK);
    const workflowWithoutCode1 = {
      ...workflowDB,
      states: workflowDB.states.map(
        ({ code: _, constraints: __, ...rest }) => rest,
      ),
    };
    const workflowWithoutCode2 = {
      ...fakeWorkflow,
      states: fakeWorkflow.states.map(
        ({ code: _, constraints: __, ...rest }) => rest,
      ),
    };
